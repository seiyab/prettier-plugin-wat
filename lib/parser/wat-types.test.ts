...
describe("valtype", () => {
	test("i32", () => {
		...
		expect(out.node).toEqual(o({ type: "NumberType", value: "i32" }));
	});
	test("i64", () => {
		const out = check(valtype.parse(input("i64")));
		expect(out.node).toEqual(o({ type: "NumberType", value: "i64" }));
	});
	test("f32", () => {
		const out = check(valtype.parse(input("f32")));
		expect(out.node).toEqual(o({ type: "NumberType", value: "f32" }));
	});
	test("f64", () => {
		const out = check(valtype.parse(input("f64")));
		expect(out.node).toEqual(o({ type: "NumberType", value: "f64" }));
	});
});
